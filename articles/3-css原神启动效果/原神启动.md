...
### fixed
将 ```position``` 设为 ```fixed``` 则会相对于当前整个网页进行定位。```top```则表示从整个网页的最上面到当前元素的距离。
```left```则代表从整个网页的最左边到当前元素的距离。```bottom```,```right``` 以此类推，分别代表最下方和最右方。
...
将 ```position``` 设为 ```absolute``` 则会相对于祖宗设置为 ```relative``` 的元素进行定位。```top```则表示从这个元素的最上面到当前元素的距离。
